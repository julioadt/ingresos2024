...
<%  
 if(request.getParameter("listar") != null && request.getParameter("listar").equals("listar")){
   ...
   rs = st.executeQuery("select * from postulantes inner join ciudades on postulantes.ciudad=ciudades.id;");
   while(rs.next()){
%>
<tr>
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                ...
                <a class="dropdown-item"
                   href="jsp/reportes.jsp?reporte=postulantes2&codigo=<%out.print(rs.getString(1)); %>"
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try{
      ...
      ResultSet rs = null;
      st = conn.createStatement();
      rs = st.executeQuery("SELECT * FROM postulantes WHERE ci='" + ci + "'");
      rs.next();
      if(rs.getRow()!=0){
        out.print("Ya existe un postulante con el C.I. ingresado");
        rs.close();
        return;
      }
      st.executeUpdate("insert into postulantes (nombre,apellido,ciudad,ci)values('"+nombre+"','"+apellido +"', '"+ciudad+"','"+ci+"')");
      st.close();
      out.println("Postulante guardado");
}catch (Exception e) {
out.println("error PSQL" + e);
}
}
   ...
   try {
        ...
        st.executeUpdate("update postulantes set nombre='" + nombre +"', apellido='"
      +apellido+"', ci='" + ci +"', ciudad='"
      +ciudad+ "' where id='" + pk + "'");
           st.close();
   ...
   ResultSet rs = null;
   st = conn.createStatement();
   rs = st.executeQuery("select * from postulantes;");
   while(rs.next()){
%>
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from postulantes where id='" + pk + "'");
