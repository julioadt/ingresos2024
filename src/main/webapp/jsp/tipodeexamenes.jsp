...
<%  
 if(request.getParameter("listar") != null && request.getParameter("listar").equals("listar")){
   ...
   rs = st.executeQuery("select * from tipo_deexamenes;");
   while(rs.next()){
%>
<tr>
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-menu">
                ...
                <a class="dropdown-item"
                   href="jsp/reportes.jsp?reporte=tipodeexamenes2&codigo=<%out.print(rs.getString(1)); %>"
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try{
      ...
      ResultSet rs = null;
      st = conn.createStatement();
      st.executeUpdate("insert into tipo_deexamenes (nombre) values('" + nombre + "')");
...
out.println("error PSQL" + e);
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("update tipo_deexamenes set nombre='" + nombre +"' where id='" + pk + "'");
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from tipo_deexamenes where id='" + pk + "'");
