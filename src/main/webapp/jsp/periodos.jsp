...
<%  
 if(request.getParameter("listar") != null && request.getParameter("listar").equals("listar")){
   ...
   rs = st.executeQuery("select * from periodos;");
   while(rs.next()){
%>
<tr>
                            ...
                            <div class="dropdown-menu">
                                ...
                                >
                                 <a class="dropdown-item"
                                    href="jsp/reportes.jsp?reporte=periodo&codigo=<%out.print(rs.getString(1)); %>"
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try{
      ...
      ResultSet rs = null;
      st = conn.createStatement();
      st.executeUpdate("insert into periodos (nombre, estado) values('" + nombre + "', '" + estado + "')");
...
out.println("error PSQL" + e);
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("update periodos set nombre='" + nombre +"', estado='" + estado + "' where id='" + pk + "'");
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from periodos where id='" + pk + "'");
