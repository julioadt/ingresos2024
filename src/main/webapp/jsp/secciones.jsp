...
<%  
 if(request.getParameter("listar") != null && request.getParameter("listar").equals("listar")){
   ...
   rs = st.executeQuery("select * from secciones;");
   while(rs.next()){
%>
<tr>
            ...
            <div class="dropdown-menu">
                ...
                >
                 <a class="dropdown-item"
                                    href="jsp/reportes.jsp?reporte=secciones2&codigo=<%out.print(rs.getString(1)); %>"
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try{
      ...
      ResultSet rs = null;
      st = conn.createStatement();
      st.executeUpdate("insert into secciones (nombre)values('"+nombre+"')");
...
out.println("error PSQL" + e);
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("update secciones set nombre='" + nombre +"' where id='" + pk + "'");
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from secciones where id='" + pk + "'");
