...
<%  
 if(request.getParameter("listar") != null && request.getParameter("listar").equals("listar")){
   ...
   rs = st.executeQuery("select * from pelotones inner join compañia on pelotones.compañia=compañia.id inner join instructores on pelotones.instructor=instructores.id;");
   while(rs.next()){
%>
<tr>
...
<%
  ...
  out.println("error PSQL"+ e); 
   ...
   try{
      ...
      ResultSet rs = null;
      st = conn.createStatement();
      st.executeUpdate("insert into pelotones (nombre , compañia, instructor) values('" + nombre + "','" + compañia + "','" + instructor + "')");
...
out.println("error PSQL" + e);
   ...
   try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("update pelotones set nombre='" + nombre +"' , compañia='" + compañia +"', instructor='" + instructor +"' where id='" + pk + "'");
...
String compa = request.getParameter("compa");
   ...
   ResultSet rs = null;
   st = conn.createStatement();
   rs = st.executeQuery("select * from pelotones where compañia='" + compa +"';");
   while(rs.next()){
%>
